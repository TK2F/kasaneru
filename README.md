...
ドック右上の **LIVE / OFFLINE** トグルで配信画面への表示をまとめて制御できます。OFFLINE にすると何も表示されません。

### チャプター

配信の進行を区切るカード。タイトルとサブタイトルを設定して、表示ボタンで切り替えます。左右の矢印で前後の章に移動できます。

### テロップカード

画面下部に表示されるテロップ。発表者名やトピック名の表示に。SHOW / HIDE で切り替えます。

### 画像

`images/` フォルダに画像を入れ、ドックから追加。スライド資料の表示などに使えます。グループごとに表示位置やアニメーションを設定できます。
...
| アプリ | 操作 |
|--------|------|
| **タイマー** | ドックの操作パネルで時間を設定し、Start / Pause / Reset で制御。カウントダウン・カウントアップに対応 |
| **時計** | 追加するだけで表示。操作パネルでタイムゾーン（UTC オフセット）を変更可能 |
| **ミュージックプレイヤー** | 操作パネルで音楽ファイルをプレイリストに追加して BGM 再生。ファイルは kasaneru フォルダ内に置くか `file:///` パスで指定 |
| **ルーレット** | 操作パネルで選択肢を入力し Spin で回転。手動で Stop して結果を決定。ホイールとリールの 2 モード |
| **ラクガキ Draw** | ドックの操作パネルに描画エリアが表示。ペン・図形・ピン・テキストで画面に書き込み。OBS の WebSocket を有効にすると、配信画面のプレビューを見ながら書き込めます（操作パネル内の手順を参照） |
| **バナー** | スクロールまたは固定テキストを表示。操作パネルでテキスト・速度・位置を設定 |
| **TRPG ダイス** | ダイスノーテーション（例: `2d6+3`）を入力して Roll。じゃんけんモード付き |
...
Kasaneru を使っていくなかで、「こういう表示もほしいな」「もうちょっと凝りたいな」と思うことがあるかもしれません。そのときは、ぜひ次のステップにも挑戦してみてください。

### 自分だけのスタイルを作る

CSS をちょっといじるだけで、配信画面の雰囲気はガラッと変わります。色を変えたり、フォントを替えたり、カードの形を工夫したり。
ご自身の配信に合うスタイルを試行錯誤するのも楽しいかと思うのでカスタマイズにもチャレンジしてみてください✨

詳しくは [CUSTOM-CSS-GUIDE.md](CUSTOM-CSS-GUIDE.md) を参照してください。

### ミニアプリを作ってみる

HTML と少しの JavaScript が書ければ、自分だけの配信ツールを作ることができます。
「カウンター」「お題ガチャ」「コメントビューア」——アイデア次第で何でも作れます。テンプレート (`templates/app/hello-world.html`) を動かすところから始めてみてください。

開発仕様は [docs/APP-SPEC.md](docs/APP-SPEC.md) に書いてあります。

### OBS のプラグインを探してみる

OBS には世界中の開発者が作った便利なプラグインがたくさんあります。シーン切り替えの演出、字幕の自動生成、配信のスケジュール管理など、使ってみると「こんなこともできるんだ」と発見があるはずです。
OBS の操作に慣れてきたら、プラグインマネージャーや [OBS Forum](https://obsproject.com/forum/resources/) を覗いてみてください。

自分の手で配信をカスタマイズしていく過程は、配信そのものと同じくらい面白いかもしれません。
...
**Kasaneru** is a lightweight overlay toolkit for OBS Studio. No build tools, no CDN — just unzip and add to OBS.
...
See [README.en.md](README.en.md) for the full English documentation, including CSS customization, app development, and OBS plugin tips.
