...
CSS をちょっといじるだけで、配信画面の雰囲気はガラッと変わります。色を変えたり、フォントを替えたり、カードの形を工夫したり。正解はないので、自分の配信に合うスタイルを試行錯誤するのも楽しいです。

詳しくは [CUSTOM-CSS-GUIDE.md](CUSTOM-CSS-GUIDE.md) を参照してください。

### ミニアプリを作ってみる

HTML と少しの JavaScript が書ければ、自分だけの配信ツールを作ることができます。「カウンター」「お題ガチャ」「コメントビューア」——アイデア次第で何でも作れます。テンプレート (`templates/app/hello-world.html`) を動かすところから始めてみてください。

開発仕様は [docs/APP-SPEC.md](docs/APP-SPEC.md) に書いてあります。

### OBS のプラグインを探してみる

OBS には世界中の開発者が作った便利なプラグインがたくさんあります。シーン切り替えの演出、字幕の自動生成、配信のスケジュール管理など、使ってみると「こんなこともできるんだ」と発見があるはずです。OBS の操作に慣れてきたら、プラグインマネージャーや [OBS Forum](https://obsproject.com/forum/resources/) を覗いてみてください。
